...
def generate_sscd_model(A, B, C, Ad, Bd, K, L, h, safex, x0_bounds, filename, mdadt1, locn, Kmax, mmax, 
                        fixed_step=0.00005, rem=1e-3, cutoff=1e-8, prec=53, orders=[3, 5]):
    """
    Generate Flow* .model file for each subsystem
    ...
    locations = [10**locn]  # e.g. locn=3 -> [1000]
    ...
    #### -----------Build file-------------- ####
    modelfile = f"{filename}_1{'0'*locn}.model"
    with open(modelfile, "w") as f:
        ...
        f.write(f"  max jumps {Kmax/(locn+1)+2}\n")
        f.write("  print off\n")
        f.write(" }\n\n")
    ...
    print(f".model file for sscd {10**locn} written to {modelfile}")
    return modelfile